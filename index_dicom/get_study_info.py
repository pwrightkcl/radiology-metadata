...
def main(input_dir: str):
    """Extract study information from DICOM files from Insight46
    ...
    The first .dcm file in the first series directory is read to extract the study information.
    
    Args:
        input_dir (str): Path to source directory containing DICOM files.
    
    Outputs:
        study_info_{timestamp}.csv: CSV file containing study information.
    """
    print(f"Extracting study info from {input_dir}")
    input_path = Path(input_dir)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    temp_csv = input_path / f"study_info_long_{timestamp}.csv"
    ...
    for study_dir in input_path.glob('20??????/1.*'):
        for series_dir in study_dir.glob('1.*'):
            ...
            print(f"No series directory found in {study_dir}")
            continue
    ...
    wide_csv = input_path / f"study_info_{timestamp}.csv"
